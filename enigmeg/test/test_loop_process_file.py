...
def parse_inputs(proc_file):
    ...
    for idx, dseries in proc_dframe.iterrows():
        ...
        info.outfolder = dseries['output_dir']
        info.bem_sol_filename = op.join(info.outfolder, dseries['subject'], 
                                        'source_space-src.fif')
        info.src_filename = op.join(info.outfolder, dseries['subject'], 
                                    'bem_sol-sol.fif')
...
def test_process_file(tmpdir):
    ...
    test_dframe.loc[2, 'output_dir'] = '~/Desktop/TEMP' #<<<<<<<<<<<<< Change to tmpdir
    ...
    test_dframe.loc[3, 'meg_file_path'] = 'short_ctf_rest.fif'
    test_dframe.loc[3, 'eroom_file_path'] = 'short_ctf_eroom.fif'
    test_dframe.loc[3, 'output_dir'] = '~/Desktop/TEMP' #<<<<<<<<<<< Change to tmpdir
    test_dframe.loc[3, 'line_freq'] = 60        
    test_dframe.loc[3, 'trans_file'] = ctf_dat['trans']
    
    output_csv = op.join('~/Desktop/TEMP', 'process.csv')  #<<<<<<<<<<<< Change to tmpdir
